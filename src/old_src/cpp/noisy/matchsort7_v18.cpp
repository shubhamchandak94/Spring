		...
		for(int j = 0; j < maxmatch; j++)
		{
			...
			for(int l = 0; l < numdict; l++)
			{
				...
				{	
					...
					for (int64_t i = dictidx[1] - 1 ; i >= dictidx[0] ; i--)
					{
						...
						if(__builtin_popcount(ref1^(read1[rid]&maskfirst[j]))+__builtin_popcount(ref2^(read2[rid]&masklast[j])) <= thresh)
			...
			for(int l = 0; l < numdict; l++)
			{
				...
				if(ull == ull1)//checking if ull is actually the key for this bin
				{
					...
					{
						auto rid = dict[l].read_id[i];
						if(__builtin_popcount(revref1^(read1[rid]&revmaskfirst[j]))+__builtin_popcount(revref2^(read2[rid]&revmasklast[j])) <= thresh)
