...
{	
	...
	for(int i = 1; i < numreads; i++)
	{
		...
		{
			std::vector<int> dictinter, dictdiff; // intersection and symmetric difference of
							//dictionary bin			
			for(int l = 0; l < numdict; l++)
			{
				b[l] = b1&mask1[l];
				...
				{
					if(dictinter.size() == 0)
						dictinter = dict[l][b[l]];
					else
					{
						std::vector<int> tempinter;
						std::set_intersection(dictinter.begin(),dictinter.end(),dict[l][b[l]].begin(),dict[l][b[l]].end(), std::back_inserter(tempinter));
						dictinter = tempinter;
					}
					std::vector<int> tempdiff;
					std::set_symmetric_difference(dictdiff.begin(),dictdiff.end(),dict[l][b[l]].begin(),dict[l][b[l]].end(), std::back_inserter(tempdiff));
					dictdiff = tempdiff;
				}
			}
			if(dictinter.size() > 0)
			{
				for (std::vector<int>::iterator it = dictinter.begin() ; it != dictinter.end(); ++it)
				{
					int k = *it;
					hammingcount++;
			...
			std::vector<int> dictinter1, dictdiff1; // intersection and symmetric difference of
							//dictionary bin			
			for(int l = 0; l < numdict; l++)
			{
				b[l] = b2&mask1[l];
				if(dict[l].count(b[l]) == 1)
				{
					if(dictinter1.size() == 0)
						dictinter1 = dict[l][b[l]];
					else
					{
						std::vector<int> tempinter;
						std::set_intersection(dictinter1.begin(),dictinter1.end(),dict[l][b[l]].begin(),dict[l][b[l]].end(), std::back_inserter(tempinter));
						dictinter1 = tempinter;
					}
					std::vector<int> tempdiff;
					std::set_symmetric_difference(dictdiff1.begin(),dictdiff1.end(),dict[l][b[l]].begin(),dict[l][b[l]].end(), std::back_inserter(tempdiff));
					dictdiff1 = tempdiff;
				}
			}
			if(dictinter1.size() > 0)
			{
				for (std::vector<int>::iterator it = dictinter1.begin() ; it != dictinter1.end(); ++it)
				{
					int k = *it;
					hammingcount++;
			...
			if(dictdiff.size() > 0)
			{
				for (std::vector<int>::iterator it = dictdiff.begin() ; it != dictdiff.end(); ++it)
				{
					int k = *it;
					hammingcount++;
					if((b1^(read[k]&mask[j])).count()<=thresh)
			...
			if(dictdiff1.size() > 0)
			{
				for (std::vector<int>::iterator it = dictdiff1.begin() ; it != dictdiff1.end(); ++it)
				{
					int k = *it;
					hammingcount++;
					if((b2^(read[k]&revmask[j])).count()<=thresh)
