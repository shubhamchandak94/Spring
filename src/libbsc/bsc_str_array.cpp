...
// put bsize bytes from buf into str_array
  ...
  while(true) {
    ...
    if(str_lengths[pos_in_str_array]-pos_in_current_str <= (uint32_t)(bsize-bytes_read)) {
      // buffer has more bytes than current string can hold
      memcpy(&str_array[pos_in_str_array][pos_in_current_str], buf+bytes_read, str_lengths[pos_in_str_array]-pos_in_current_str);
      ...
      memcpy(&str_array[pos_in_str_array][pos_in_current_str], buf+bytes_read, bsize-bytes_read);
...
void Compression(char * argv[])
{
    ...
    BSC_FILEOFFSET fileSize = std::accumulate(str_lengths,str_lengths+size_str_array,0);
...
public:
int bsc_main(int argc, char * argv[], std::string *str_array_param, const uint32_t size_str_array_param, uint32_t *str_lengths_param)
{
...
void BSC_str_array_compress(const char *outfile, std::string *str_array_param, const uint32_t size_str_array_param, uint32_t *str_lengths_param, const int bsize/* = BSC_BLOCK_SIZE*/) {
  ...
  std::vector<std::string> arguments = {"", "e", "", outfile, "-b" + std::to_string(bsize), "-p", "-e1"};
  std::vector<char*> argv;
  for (const auto& arg : arguments)
    argv.push_back((char*)arg.data());
  argv.push_back(nullptr);
  b.bsc_main(argv.size() - 1, argv.data(), str_array_param, size_str_array_param, str_lengths_param);
}
