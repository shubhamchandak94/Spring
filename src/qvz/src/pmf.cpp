...
uint32_t is_pmf_valid(struct pmf_t *pmf) {
  ...
  if (!pmf->pmf_ready) recalculate_pmf(pmf);

  for (i = 0; i < pmf->alphabet->size; ++i) {
    sum += pmf->pmf[i];
  }

  if (fabs(sum - 1.0) < 0.0001) return 1;
  return 0;
}
...
double get_symbol_probability(struct pmf_t *pmf, symbol_t symbol) {
  ...
  if (!pmf->pmf_ready) recalculate_pmf(pmf);
  if (idx != ALPHABET_SYMBOL_NOT_FOUND) return pmf->pmf[idx];
  return 0.0;
}
...
double get_kl_divergence(struct pmf_t *p, struct pmf_t *q) {
  ...
  if (p->alphabet != q->alphabet) return NAN;

  if (!p->pmf_ready) recalculate_pmf(p);
  if (!q->pmf_ready) recalculate_pmf(q);
...
struct pmf_t *combine_pmfs(struct pmf_t *a, struct pmf_t *b, double weight_a,
  ...
  if (!a->pmf_ready) recalculate_pmf(a);
  if (!b->pmf_ready) recalculate_pmf(b);
...
void alphabet_union(const struct alphabet_t *restrict a,
  ...
  if (result->symbols) free(result->symbols);
