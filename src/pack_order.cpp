...
int pack_order(std::string &temp_dir, bool &paired_end)//pack order into least number of bits possible
{
	std::string infile;
	std::string infilenumreads;
	uint32_t numreads;
	std::string paired_end = std::string(argv[2]);
	infile = temp_dir + "/read_order.bin";
	infilenumreads = temp_dir + "/numreads.bin";
